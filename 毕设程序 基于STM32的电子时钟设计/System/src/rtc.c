...
u8 RTC_Get(void){//读出当前时间值 //返回值:0,成功;其他:错误代码.
	...
	u32 temp=0;
	u16 temp1=0;
	timecount=RTC_GetCounter();		                     //从RTC寄存器中读出计数值，RTC在后备区，只要不掉电数据就不会被清楚
...
u32 Get_ALR_Value(u16 syear,u8 smon,u8 sday,u8 hour,u8 min,u8 sec){ //写入当前时间（1970~2099年有效），
	u16 t;
	u32 seccount=0;
	if(syear<2000||syear>2099)return 1;//syear范围1970-2099，此处设置范围为2000-2099       
	for(t=1970;t<syear;t++){ //把所有年份的秒钟相加
...
//	RTC_SetCounter(seccount);//把换算好的计数器值写入
//	RTC_WaitForLastTask(); //等待写入完成
	return seccount; //返回值:0,成功;其他:错误代码.    
}
